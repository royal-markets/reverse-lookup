...
import placeholderImg from "@/assets/fallback.png";
    ...
    <div className="flex flex-col border  rounded-3xl overflow-hidden shadow-lg max-w-full">
      ...
      <div className="flex p-4 items-center space-x-4 bg-white">
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <p className="text-xs md:text-sm text-base-500 truncate">{url}</p>
        </div>
      </div>
    </div>
  );
...
export function IosOgShellCard({ children }: { children: ReactNode }) {
  return (
    <div className="max-w-xs md:max-w-md mx-auto flex flex-col rounded-lg bg-base-975 px-px pb-px shadow-inner-shadow">
      <div className="p-4 flex flex-col md:px-5">
        <div className="mb-2 text-sm md:text-base text-base-500">iMessage</div>
        <div className="mb-3 text-xs md:text-sm text-base-500">Today 11:29</div>
        <div className="ml-auto px-4 py-2 mb-3 text-white bg-blue-500 rounded-2xl">
          <span>Check out this new product!</span>
        </div>
        {children}
        <div className="mt-3 text-xs md:text-sm text-base-500">Delivered</div>
      </div>
    </div>
  );
}
