// Credit - https://github.com/mfts/gauge-demo
...
export const Gauge = ({
  ...
  return (
    <div className="flex flex-col items-center justify-center relative bg-black rounded-full shadow-xl border border-black">
      <svg
        fill="none"
      ...
      {showValue ? (
        <div className="absolute flex opacity-0 animate-gauge_fadeIn">
          <p className={`text-base-100 ${sizes[size].textSize}`}>{value}</p>
        </div>
      ) : null}
      {alternateValue ? (
        <div className="absolute flex opacity-0 animate-gauge_fadeIn ">
          <p className={`text-base-100 ${sizes[size].textSize}`}>
            {alternateValue}
          </p>
        </div>
      ) : null}
